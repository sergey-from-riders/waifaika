...
## 2026-03-17 22:55 MSK

### Done
- Основной `README.md` переведён на русский и оформлен как дефолтная публичная витрина репозитория.
- Добавлен полный английский вариант `README.en.md` с тем же структурным покрытием: overview, screenshots, stack, quick start и repo notes.
- Подготовлен `docs/community-posts.md` с готовыми шаблонами для разных площадок:
  - Telegram / VK
  - X / Threads
  - Habr / vc / LinkedIn
  - Show HN
  - Reddit / dev.to
  - короткие repo descriptions на русском и английском
- Итерация локальная-only: runtime и продовая сборка не менялись, поэтому повторный деплой не выполнялся.
- Проверки завершены успешно:
  - `cd backend && GOTOOLCHAIN=go1.25.8+auto /usr/local/go/bin/go test ./...`
  - `cd web && npm test -- --run`
  - `cd web && npm run build`
...
### Next
- При желании можно отдельно собрать ещё пакет анонсов под Product Hunt, DEV Community и email outreach.
- Если захочется ещё сильнее локализовать публичную подачу, можно перевести на русский и `CONTRIBUTING` / `SECURITY`, оставив английские копии рядом.

### Risks
- Community post templates универсальные и рассчитаны на быстрый старт; перед публикацией в конкретное сообщество лучше слегка адаптировать тон и длину под площадку.
- Основной `README.md` теперь русскоязычный; для англоязычной аудитории нужно вести трафик на `README.en.md` или закрепить language switch в описании репозитория.
