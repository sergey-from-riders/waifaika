...
Wifiyka is a mobile-first map/PWA for crowdsourced Wi-Fi discovery. The repository is structured as a monorepo with a React/Vite frontend, a Go API, PostgreSQL/PostGIS storage, offline pack support via PMTiles, and production deployment templates for `nginx + systemd`.

This project can also be used as a base for other location-heavy applications: city guides, field collection tools, offline map viewers, place directories, local review maps, and similar products where map interaction, sync, and resilient mobile UX matter.
...
- Deploy: `nginx`, `systemd`, Docker Compose for local infrastructure
...
- `deploy/offline-packs/sochi.pmtiles` is a demo/offline asset kept in the repository intentionally.
- `backend/internal/static/webdist/` contains embedded frontend artifacts needed for backend fallback serving.
- `deploy/legal/*.txt` is intentionally externalized and gitignored so each installation can inject its own legal text without leaving it in repository history.
...
- API docs are available both as a raw spec (`/openapi.yaml`) and as a browser UI (`/api-docs.html`, `/swagger`).

For setup, production release flow, and reuse guidance, see the documents above.
